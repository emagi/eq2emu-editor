...
if( (isset($_GET['search']) && $_GET['search'] != '') && (isset($_GET['type']) && $_GET['type'] != '') ) 
{
	...
	switch($_GET['type'])
	{
		...
		case "lookup":
			$sql = "SELECT distinct(name) as search_text FROM ".$GLOBALS['db_name'].".quests WHERE name like '".$search."%' OR description like '".$search."%' OR zone like '".$search."%' OR type like '".$search."%' ORDER BY name";
			break;
