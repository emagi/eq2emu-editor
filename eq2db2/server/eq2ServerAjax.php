...
if( (isset($_GET['search']) && $_GET['search'] != '') && (isset($_GET['type']) && $_GET['type'] != '') ) 
{
	...
	switch($_GET['type'])
	{
		...
		case "lookup":
			$sql = "SELECT distinct(name) as search_text FROM ".$GLOBALS['db_name'].".characters WHERE name like '".$search."%' ORDER BY name";
			break;
		case "entity_commands":
			$sql = "SELECT distinct(command_text) as search_text FROM " . $GLOBALS['db_name'] . ".entity_commands WHERE command_text LIKE '" . $search . "%' ORDER BY command_text";
			break;
		default:
			break;
	}
